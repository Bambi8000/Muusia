...
- 2026-09-12 — Fixed manual registration being blocked by extra photos: Review markers no longer requires the photo count to match, each photo has a direct Place markers manually action, and Register includes extras while extraction still uses only required sheets. Two failed detections, four-point manual placement on the extra photo, retained positions and 390 px layout verified in the production browser; all 54 tests pass.
- 2026-09-12 — Added optional Auto adjust (paper light, white balance and bounded whole-photo contrast) and Sharpen with 0–100% strength, local halo/noise limits and a sheet-space radius. Preview and extraction share processing, threshold stays last, and changed options invalidate crops. Reference-photo playback/GIF, reset and mobile layout verified; all 59 tests pass. Faint paper residue improves, but distinct photographed double lines or missing focus detail are not reconstructed.
- 2026-09-14 — Added shared Paper color (White / light or Black / dark) in Sheet and Adjust. Dark mode detects bright hatches/dark holes, subtracts smooth background glare, uses bounded whole-photo contrast, preserves colored ink without white-reference correction, and uses black padding through extraction/export. Bright-drawing stabilization and preview invalidation follow the same setting. Confirmed IMG_2609.jpeg A3/4×3/30 mm margin/8 mm gap/15 mm markers; the supplied first sheet finds all four markers and yields 12 full-resolution crops plus GIF/MP4/PNG ZIP in the production browser. Dark/light switching, reset and 390 px layout verified; all 70 tests pass. The supplied original photo remains outside the repository.
- 2026-09-14 — Implemented the save/load part of M7. Save project downloads a single `.liike` ZIP containing versioned project JSON and the original photo bytes, extending the original settings-only proposal for a self-contained round trip. Restores all sheet/adjustment/sequence/export controls, markers and photo order, including unfinished work. Loading validates and stages photos before replacing the session; new photo IDs and remapped frame references prevent stale frames, and failure/cancellation cleans up staged resources. The real-photo two-sheet test saved to disk, reloaded into an empty session and rebuilt its manual/excluded sequence successfully; all 79 tests pass, with mobile layout verified. Random-order seed UI and other multi-sheet polish remain future work.
- 2026-09-15 — Local close-up capture proof: the new cell-outline/external-ring detector correctly registers supplied IMG_2635.jpeg and IMG_2636.jpeg, including all right-angle rotations and partial neighboring cells. Fourteen new synthetic tests check light/dark paper, perspective, rotation, noise and graceful rejection, with corner recovery within 0.3 mm; all 93 tests and TypeScript/lint pass. Generated two 2160×1888 lossless review crops from original pixels using a shared 1 mm cell inset. The narrower frame-window crop would clip upper strokes in these particular drawings. Detector/UI integration, capture-mode geometry, paper adjustment references and persistence remain to be implemented; this is a tested prototype, not a Pages release. Photos and review outputs remain outside tracked files. The root KELA-HANDOFF.md supplies the newer close-up geometry contract; its scaffold/naming/milestone assumptions do not replace this app's completed implementation history.

- 2026-09-15 — Integrated Individual frames capture mode for publication: one outlined cell per photo, automatic external-ring orientation, manual corner fallback/rotation, cell-local perspective geometry, shared Full cell crop with independent border trim and plot clearance, and 2160 px default output. Close-up paper references use the blank inner band and shared contrast limits. Save format v2 records capture geometry and registration mode while migrating v1 sheet projects unchanged. Original bitmaps are decoded on demand; the 1080 px adjustment preview now samples original pixels. The supplied two photos passed automatic registration, manual fallback/re-detection, extraction, lossless PNG export and project download/restore with byte-identical originals. All 102 tests and strict TypeScript/lint pass. This supersedes the earlier unintegrated-prototype status; original photos and generated QA files remain outside Git.

- 2026-09-15 — Corrected misleading close-up numbering: upload positions are Photo N, while Frame number on paper is explicit per-photo metadata. Register no longer overlays 1 on an unnumbered first photo. Number entry is available in Photos and Register, and Order by frame number sorts photos without changing corners/crops. Labels, sequence and PNG source metadata retain assigned numbers; project v3 saves them and migrates v1/v2 with unknown numbers unset. Automatic detection still reads only outline/orientation, not printed digits. All 107 tests and production builds pass; browser assignment/sorting/export/save-load verified.

- 2026-09-16 — Added local recognition of Muusia's printed frame numbers, including labels crossing the cell outline, with automatic reads on close-up import and Read number / Read missing numbers in Photos, Register and Sequence. Numeric close-up ordering now feeds playback and all exporters; unknown/duplicate numbers require correction, exclusion or explicit Photo/Manual order. Number edits reuse existing crops. Project v4 migrates older close-up Original order to Frame number while retaining manual order. All 24 labels in geid2.liike were read correctly with saved corners; browser numeric/reverse/ping-pong/manual ordering, 390 px controls, PNG ZIP source order and v4 download/restore were verified. All 124 tests and both production builds pass. The pre-existing outline detector still requires manual corners for five of these 24 photos.

- 2026-09-17 — Added Chat GIF (480 px) and Smaller GIF (320 px) export presets with a shared 64-color palette, no dithering and infinite loop; frame order, all frames and playback speed are retained. Added 64/128/256 GIF color selection and backward-compatible project persistence (older files retain 256 colors). Browser exports of the 24-frame geid2 reel measured 1.56 MB and 0.75 MB versus 10.26 MB at 1080 px; encoded dimensions, frames, timing and loop, visible line detail and 390 px controls verified. All 127 tests and both builds pass. Chat-app sending was not exercised. Daniel approved publication and requested that future completed and tested Liike updates be published directly to the existing GitHub Pages site without asking for a separate release approval.

...
type Props = { library: ReturnType<typeof usePhotos>; sheets: number; settings: SheetSettings; onBack: () => void; onRegister: (index: number) => void; onUsePhotos?: () => void };
export default function PhotosStep({ library, sheets, settings, onBack, onRegister, onUsePhotos }: Props) {
  ...
  const closeup = individualFrames(settings), noun = closeup ? 'Frame' : 'Sheet', feature = closeup ? 'corners' : 'markers';
  ...
  return <>
    ...
    <ol className="photo-list">{visiblePhotos.map((photo, i) => <li key={photo.id}>
      <img src={photo.url} alt={`${noun} ${i + 1}: ${photo.name}`} />
      <div className="photo-details"><p className="eyebrow">{noun} {i + 1}{i >= sheets ? ' · extra photo' : ''}</p><h2>{photo.name}</h2><p>{photo.width} × {photo.height} px · {photo.points.filter(Boolean).length}/4 {feature} placed</p>{photo.detection && <p role="status">{photo.detection.message}</p>}

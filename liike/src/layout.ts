...
export function validateSettings(p: SheetSettings): string[] {
  ...
  if (p.captureMode !== undefined && !['sheet', 'frames'].includes(p.captureMode)) issues.push('Choose a capture mode.');
  if (!Number.isFinite(p.clearance ?? 0) || (p.clearance ?? 0) < 0 || (p.clearance ?? 0) > 10) issues.push('Clearance must be between 0 and 10 mm.');
  if (!Number.isFinite(p.trim ?? 0) || (p.trim ?? 0) < 0 || (p.trim ?? 0) > 10) issues.push('Border trim must be between 0 and 10 mm.');
  ...
  const inner = Math.min((p.W - 2 * p.margin - (p.cols - 1) * p.gap) / p.cols, (p.H - 2 * p.margin - (p.rows - 1) * p.gap) / p.rows);
  if (inner - 2 * Math.max(p.clearance ?? 0, p.trim ?? 0) <= 1) issues.push('Reduce clearance or border trim so the frame remains larger than 1 mm.');
  return issues;
}
...
export function buildLayout(p: SheetSettings) {
  ...
  const scale = Math.min((cw - 2 * (p.clearance ?? 0)) / p.W, (ch - 2 * (p.clearance ?? 0)) / p.H);
  const cells = Array.from({ length: p.cols * p.rows }, (_, index) => {
    ...
    const trim = p.crop === 'Full cell' ? (p.trim ?? 0) : 0;
    const base = p.crop === 'Frame window' ? window : { x: cell.x + trim, y: cell.y + trim, w: cw - 2 * trim, h: ch - 2 * trim };

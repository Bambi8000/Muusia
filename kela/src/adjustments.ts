...
export type Adjustments = { flatten: boolean; whiteBalance: boolean; black: number; white: number; gamma: number; saturation: number; thresholdEnabled: boolean; threshold: number };
export const DEFAULT_ADJUSTMENTS: Readonly<Adjustments> = Object.freeze({ flatten: true, whiteBalance: true, black: 0, white: 255, gamma: 1, saturation: 100, thresholdEnabled: false, threshold: 128 });
export type PaperModel = { W: number; H: number; h: Matrix; photoWidth: number; photoHeight: number; paper: number[]; coefficients: number[][]; samples: number; supported: boolean; note: string };
...
export function validateAdjustments(a: Adjustments) {
  if (![a.black, a.white, a.gamma, a.saturation, a.threshold].every(Number.isFinite) || a.black < 0 || a.black > 120 || a.white < 135 || a.white > 255 || a.gamma < .3 || a.gamma > 3 || a.saturation < 0 || a.saturation > 200 || a.threshold < 0 || a.threshold > 255) throw new Error('Check the color adjustment values.');
}
...
export function estimatePaper(source: Raster, original: { width: number; height: number }, h: Matrix, settings: SheetSettings): PaperModel {
  ...
  return { W, H, h, photoWidth: original.width, photoHeight: original.height, paper: [0, 1, 2].map(c => median(samples.map(s => s.rgb[c]!))), coefficients, samples: samples.length, supported: true, note: 'Lighting estimated from blank paper across this sheet. The same correction is used for every frame.' };
}
...
export function adjustRaster(source: Raster, rect: Rect, model: PaperModel, adjustments: Adjustments): Raster {
  ...
  const peak = Math.max(...model.paper), wb = model.paper.map(value => peak / Math.max(20, value));
  const targets = model.paper.map(value => value * 245 / peak);
  const flatten = a.flatten && model.supported, whiteBalance = a.whiteBalance && model.supported;
    ...
    for (let x = 0; x < source.width; x++) {
      ...
      const rgb = [0, 1, 2].map(c => {
        ...
        if (valid && flatten) {
          ...
          value *= clamp(targets[c]! / Math.max(20, field), .25, 4);
        }
        if (valid && whiteBalance) value *= wb[c]!;
        return Math.pow(clamp((value - a.black) / (a.white - a.black), 0, 1), 1 / a.gamma) * 255;
      });
      const light = .2126 * rgb[0]! + .7152 * rgb[1]! + .0722 * rgb[2]!;
      for (let c = 0; c < 3; c++) data[i + c] = a.thresholdEnabled ? (light < a.threshold ? 0 : 255) : clamp(light + (rgb[c]! - light) * a.saturation / 100);
      data[i + 3] = 255;
    }
  }

...
type Props = { library: ReturnType<typeof usePhotos>; sheets: number; settings: SheetSettings; onBack: () => void; onRegister: () => void };
export default function PhotosStep({ library, sheets, settings, onBack, onRegister }: Props) {
  ...
  return <>
    ...
    <div className="photo-count" role="status"><strong>{photos.length} / {sheets} photos</strong><span>{remaining > 0 ? `Add ${remaining} more ${remaining === 1 ? 'photo' : 'photos'} for the complete sequence.` : remaining < 0 ? 'Remove extra photos or increase the total frame count in Sheet.' : 'All sheets have a photo.'}</span></div>
    ...
    <ol className="photo-list">{photos.map((photo, i) => <li key={photo.id}>
      ...
      <div className="photo-details"><p className="eyebrow">Sheet {i + 1}{i >= sheets ? ' · extra photo' : ''}</p><h2>{photo.name}</h2><p>{photo.width} × {photo.height} px · {photo.points.filter(Boolean).length}/4 markers placed</p>{photo.detection && <p role="status">{photo.detection.message}</p>}
        <div className="photo-actions"><button className="secondary" disabled={busy} onClick={() => { replace.current = photo.id; if (input.current) { input.current.multiple = false; input.current.click(); } }}>Replace</button><button className="secondary" disabled={busy} onClick={() => remove(photo.id)}>Remove</button><button className="icon-button" disabled={busy || i === 0} aria-label={`Move sheet ${i + 1} earlier`} onClick={() => move(photo.id, -1)}>↑</button><button className="icon-button" disabled={busy || i === photos.length - 1} aria-label={`Move sheet ${i + 1} later`} onClick={() => move(photo.id, 1)}>↓</button></div>
      </div>
    </li>)}</ol>
    <div className="step-actions"><button className="secondary" disabled={busy} onClick={onBack}>← Sheet settings</button><button className="primary" disabled={busy || photos.length === 0 || remaining < 0} onClick={onRegister}>Review markers →</button></div>
  </>;
}

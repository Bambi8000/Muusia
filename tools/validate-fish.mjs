/* Validator for the Fish node. Run from the repo root: node tools/validate-fish.mjs
...
ok(b1.paths.length === nFishDefault, "bodies: one closed silhouette per fish (" + b1.paths.length + " = " + nFishDefault + ")");
...
/* --- point count / per fish sanity --- */
const oneD = run({ ...p0, style: "Detailed", count: 1, rows: 1 });
const oneS = run({ ...p0, style: "Simple", count: 1, rows: 1 });
ok(npts(oneD) > npts(oneS) * 1.5, "Detailed draws far more than Simple (" + npts(oneD) + " vs " + npts(oneS) + ")");
ok(oneS.paths.length >= 3 && oneS.paths.length <= 40, "Simple fish is a handful of strokes (" + oneS.paths.length + ")");
...
diff({ species: "Pike" }, "species (Detailed)");
...
diff({ scales: false }, "scales (Detailed)");
diff({ mouth: "Open" }, "mouth");
diff({ bubbles: false }, "bubbles");
...
for (const sp of def.params.find((q) => q.key === "species").options) {
  const r = run({ ...p0, style: "Detailed", species: sp, count: 1, rows: 1, wobble: 0 });
  const bb = bbox(r);
...
{
  const pr = { ...p0, count: 1, rows: 1, jitter: 0, wobble: 0, style: "Detailed", species: "Perch" };
...
/* --- shrink only: a small fish in a huge cell never grows --- */
{
  const r = run({ ...p0, count: 1, rows: 1, size: 12, wobble: 0, style: "Detailed", species: "Generic", bubbles: false, jitter: 0 });
  const bb = bbox(r);
...
/* --- rows grid: fish sit in distinct cells --- */
{
...
/* --- school --- */
{
  ...
  ok(b.paths.length === 12, "School: 12 fish placed (" + b.paths.length + ")");
  ...
  ok(dense.paths.length === 60 && inb(dense, 297, 210, p0.margin, 1.0), "School: 60 large fish still all placed and in bounds");
  ...
  ok(minD > 8, "School: fish centres are spread out (min distance " + minD.toFixed(1) + " mm)");
  const h0 = bbox(run({ ...ps, count: 1, heading: 0, turn: 0 })), h90 = bbox(run({ ...ps, count: 1, heading: 90, turn: 0 }));
  ok(h90.h > h90.w && h0.w > h0.h, "School: heading 90 turns the fish upright");
}
...
/* --- spine --- */
{
  ...
  ok(b.paths.length === expect, "Spine: one fish per resampled point (" + b.paths.length + " = " + expect + ")");
  const unw = bodies(pw);
  ok(unw.paths.length > 1 && inb(run(pw), 297, 210, 0, 1.0), "Spine unwired: default line draws fish inside the sheet (" + unw.paths.length + ")");
  ...
  ok(Math.abs(cy(on) - 40) < 1.5, "Spine On path: fish centred on the line (y " + cy(on).toFixed(1) + ")");
  ...
  /* travel direction: a right-to-left line flips the fish */
  const back = { paths: [{ pts: [[280, 40], [20, 40]], closed: false, layer: 0 }] };
  const pd = { ...pw, space: 100, bubbles: false };
  ...
  ok(f1 && f2 && Math.abs(bx(f1) - 20) < 2.5 && Math.abs(bx(f2) - 280) < 2.5, "Spine: fish are centred on the sample points (" + bx(f1).toFixed(1) + ", " + bx(f2).toFixed(1) + ")");
  /* the tail end is wide (two lobes), the nose end is narrow: spread of y near each x extreme tells which way the fish points */
  const pdd = { ...pd, style: "Detailed", species: "Generic" };
  const g1 = bodies(pdd, undefined, ins).paths[0], g2 = bodies(pdd, undefined, [undefined, back]).paths[0];
  const spread = (q, side) => { const b = bbox({ paths: [q] }); const band = b.w * 0.08; const sel = q.pts.filter((pt) => side > 0 ? pt[0] > b.x1 - band : pt[0] < b.x0 + band); const ys = sel.map((pt) => pt[1]); return Math.max(...ys) - Math.min(...ys); };
  ok(spread(g1, 1) < spread(g1, -1) && spread(g2, -1) < spread(g2, 1), "Spine: fish face the path's travel direction (narrow nose forward, wide tail behind)");
  const vert = { paths: [{ pts: [[100, 20], [100, 190]], closed: false, layer: 0 }] };
  const bv = bbox(bodies({ ...pw, space: 500 }, undefined, [undefined, vert]));
  ok(bv.h > bv.w, "Spine: a vertical line stands the fish on end");
  ...
  ok(rc.paths.length > 0 && finiteAll(rc), "Spine: closed path hosts fish");
...
const ext = runAll({ ...p0, count: 60, rows: 12, detail: 100, scales: true, style: "Detailed", size: 200, wobble: 100 });
...
/* --- showIf --- */
{
  ...
  ok(!vS.includes("species") && !vS.includes("scales") && vD.includes("species") && vD.includes("scales"), "showIf: Species and Scales hide in Simple");
...
if (def.overlay) {
  ...
  ok(Array.isArray(g1) && g1.length === 1 + p0.count * p0.rows, "overlay: margin rect + one cell per fish (" + g1.length + ")");

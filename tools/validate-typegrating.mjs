/* Validator for the Atype Grating node. Auto-switches to the baked version
   when it exists (src/defs/nodes/typegrating.js); otherwise evaluates the lab
   file with injected helpers. Run: node tools/validate-typegrating.mjs */
...
const BAKED = path.join(ROOT, "src/defs/nodes/typegrating.js");
const LAB = path.join(ROOT, "nodes-lab/typegrating.plotternode.js");

let def;
if (fs.existsSync(BAKED)) {
  def = (await import(url.pathToFileURL(BAKED).href)).default;
  console.log("using BAKED src/defs/nodes/typegrating.js");
} else {
  ...
  console.log("using LAB nodes-lab/typegrating.plotternode.js");
}
...
/* 9d) Outline: Break sailyttaa mustetta vedon YTIMESSA (vain reunakaista katkoo) */
const OU = run({ gstyle: "Outline", mode: "Break" });
const ouIn = inkNear(OU, gp[0], gp[1], Math.min(1.2, P.sw / 2 - 2));
check("Outline: ink survives in stroke core", ouIn > 0.5, "coreInk=" + ouIn.toFixed(1));

/* 9e) Stencil: saannolliset katkot -> Break sailyttaa enemman mustetta kuin Plain,
      ja tulos on deterministinen ilman seedia (seed ei muuta Stencil+Break-tulosta) */
const stInk = totalInk(run({ gstyle: "Stencil", mode: "Break" }));
check("Stencil: periodic cuts keep more ink than Plain", stInk > plainInk * 1.01,
  stInk.toFixed(0) + " vs " + plainInk.toFixed(0));
check("Stencil: seed-independent (regular cuts)",
  JSON.stringify(run({ gstyle: "Stencil", mode: "Break", seed: 1 })) ===
  JSON.stringify(run({ gstyle: "Stencil", mode: "Break", seed: 2 })));

check("param live: slant", JSON.stringify(run({ slant: 20 })) !== JSON.stringify(run({ slant: 0 })));

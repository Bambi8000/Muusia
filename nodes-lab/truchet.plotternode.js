({
  ...
  cat: "gen",
  group: "geometric",
  desc: "The classic tile maze: every grid cell gets a randomly oriented pair of quarter-arcs (or straight chamfers), and because the strands cross cell edges at fixed stations, they join seamlessly from tile to tile - the node CHAINS them, so the whole labyrinth comes out as closed loops and long border-to-border strokes instead of thousands of tiny arcs. Strands draws 1-4 parallel lines per tile for a woven look; Tiles picks Arcs, Lines (45-degree chamfers) or Mixed per tile; Subdivide + Sub levels split seeded cells into quarter-size tiles for the multiscale look (strands break at scale seams - that is the style). Pens by depth inks each scale level with its own pen. Wire Frame into Seed to reshuffle the maze per paper.",
  ins: [Pin("style", "Style")],
  outs: [Pin("paths")],
  params: [
